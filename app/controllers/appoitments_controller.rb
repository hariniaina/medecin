class AppoitmentsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @appoitment.save
        format.html { redirect_to root_path, notice: 'Appoitment was successfully created.' }
  ...
  def destroy
    @appoitment.destroy
    respond_to do |format|
      format.html { redirect_to root_path, notice: 'Appoitment was successfully destroyed.' }
      format.json { head :no_content }
    end
  end
  def validates
    app = params[:app]
    a = Appoitment.find(app)
    a.check = true
    a.save
  ...
  private
    ...
    def appoitment_params
      params.require(:appoitment).permit(:motif, :date ,:heure , :duree, :doctor_id, :patient_id)
    end
end
